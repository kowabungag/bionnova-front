...
export const fetchCourses = createAsyncThunk(
  ...
  async (language = localStorage.getItem('language') ? localStorage.getItem('language') : 'ua') => {
    try {
      ...
      const globalProgramsByLanguage = findObjectsByLanguage(
        programsList.data.data,
        language,
        false,
        webinarList.data.data
      );
      const globalWebinarsByLanguage = findObjectsByLanguage(
        webinarList.data.data,
        language,
        false
      );
...
export const coursesSlice = createSlice({
  ...
  reducers: {
    changeGlobalCoursesLanguage(state, action) {
      ...
      state.globalCoursesByLanguage.programs = findObjectsByLanguage(
        programs,
        language,
        false,
        webinars
